...
const pressureFromElevation = (elevation: Feet, referencePressure = 1013.25 as Millibar):Millibar =>
    //
    Math.pow(-((elevation * 3.28084) / 145366.45 - 1), 1 / 0.190284) * referencePressure;
...
const dryLapse = (pressure: Millibar, temp: Celsius, p0: number) =>
    //
    temp * Math.pow(pressure / p0, SPECIFIC_GAS_CONSTANT / PRESSURE_FOR_DRY_AIR_CONSTANT);
...
function mixingRatio(partialPressure: number, totalPressure: Millibar, molecularWeightRatio = MOLECULAR_WEIGHT_RATIO) {
...
const saturationMixingRatio = (pressure: Millibar, temp: Celsius) => mixingRatio(saturationVaporPressure(temp), pressure);
...
function saturationVaporPressureAlt(tempC: number): number {
    ...
    let es: number
    if (tempC >= 0) {
        es = Math.exp(34.494 - (4924.99 / (tempC + 237.1))) / Math.pow(tempC + 105, 1.57)
    } else {
...
function moistGradientT(pressure: Millibar, temp: Kelvin) {
    ...
    const d = PRESSURE_FOR_DRY_AIR_CONSTANT + (Math.pow(HEAT_OF_VAPORIZATION, 2) * smRatio * MOLECULAR_WEIGHT_RATIO) / (SPECIFIC_GAS_CONSTANT * Math.pow(temp, 2));
...
function getElevation(pressure: Millibar, p0 = 1013.25) {
    const t0 = 288.15;
    return (t0 / L) * (Math.pow(pressure / p0, (-L * SPECIFIC_GAS_CONSTANT) / g) - 1);
}
