...
function Graphic({ width, height }: SkewT.Dimensions) {
  ...
  const interpolateTime = React.useCallback(
    (e: SkewPointerEvent) => {
      ...
      const timeIndex = clampIndex(
        Math.round(x.invert(e.offsetX)),
        data.length
      );
...
type ContainerProps = React.PropsWithChildren<
  SkewT.Dimensions & {
    setState: React.Dispatch<React.SetStateAction<SkewT.Dimensions>>;
  }
>;
type ContainerRef =
...
const Container = React.forwardRef(
  (props: ContainerProps, ref: ContainerRef) => {
    const handleResize = () => {
  ...
  return (
    ...
    >
      <div>
        <button
          onClick={() =>
            setPartialDataState({ pressureLevel: dataState.pressureLevel + 1 })
          }
        >
          +
        </button>
