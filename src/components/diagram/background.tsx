...
import { getElevation, dryLapse, mixingRatio, saturationVaporPressure, vaporPressure, dewPoint, moistGradientT } from '../../thermo';
...
function IsoTherms(props: SkewT.GElementProps): JSX.Element {
  const { dimensions: { height }, scales } = useSkewT();
  ...
  const ref = useD3<SVGGElement>(
    (g) => {
      g.selectAll('isotherms')
        ...
        .enter()
        .append('line')
        .attr('x1', (d) => scales.xLinear(d) - 0.5 + (scales.yLogarithmic(PRESSURE.BASE) - scales.yLogarithmic(PRESSURE.TOP)) / TANGENT)
        ...
        .attr('clip-path', ATTRIBUTES.CLIP_PATH);
      //
      g.selectAll('line').attr('stroke', (d) => (d === 0 || d === -20 ? 'blue' : null));
    },
    [height, scales]
  );
  ...
  const lineGenerator = React.useMemo(() => {
    ...
    return d3.line<number>()
      ...
      .x((tempC, i) => {
        ...
        return scales.xLinear(tempC) + (scales.yLogarithmic(PRESSURE.BASE) - scales.yLogarithmic(mBar)) / TANGENT;
      })
      .y((_, i) => {
        const mBar = PRESSURE.RANGE[i];
...
function EnvironmentalLapseRate(props: SkewT.GElementProps): JSX.Element {
  ...
  function elrLine(d: Millibar) {
    ...
    return scales.xLinear(tempC) + (scales.yLogarithmic(PRESSURE.BASE) - scales.yLogarithmic(d)) / TANGENT
  }

  const lineGenerator = React.useMemo(() => {
    return d3.line<Millibar>() //
      .curve(d3.curveLinear)
      .x(elrLine)
  ...
  const ref = useD3<SVGGElement>(
    (g) => {
      g
        .selectAll('elr')
        .data([PRESSURE.RANGE.filter((p) => p > PRESSURE.AT_11KM).concat([PRESSURE.AT_11KM, 50 as Millibar])])
        .enter()
        .append('path')
        .attr('d', lineGenerator)
        .attr('class', ATTRIBUTES.CLASS)
  ...
  const lineGenerator = React.useMemo(() => {
    return d3.line<number>()
      ...
      .x((tempC, i) => {
        ...
        return scales.xLinear(dryLapse(mBar, tempC - ABSOLUTE_ZERO, PRESSURE.BASE) + ABSOLUTE_ZERO) + (scales.yLogarithmic(PRESSURE.BASE) - scales.yLogarithmic(mBar)) / TANGENT
      })
      .y((_, i) => {
        const mBar = PRESSURE.RANGE[i];
        if (!mBar) return 0;
...
export { IsoTherms, IsoBars, EnvironmentalLapseRate, DryAdibats, IsoHumes, MoistAdiabats }