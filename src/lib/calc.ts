...
export function firstIntersection(x1s: number[], y1s: number[], x2s: number[], y2s: number[]) {
...
export function scaleLog(from: number[], to: number[]) {
	...
	scale.invert = (v: number) => Math.exp(sampleAt(to, from, [v])[0]);
	return scale;
}
...
export const line = (x, y) => (d: number[]) => {
	const points = d.map((v) => x(v).toFixed(1) + ',' + y(v).toFixed(1));
	return 'M' + points.join('L');
};

export const lerp = (v0: number, v1: number, weight: number) => v0 + weight * (v1 - v0);

export const zip = (a0: any[], a1: any[]) => a0.map((v, i) => [v, a1[i]]);
