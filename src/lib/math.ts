...
export function scaleLog(from: number[], to) {
...
const line = (x, y) => (d: number[]) => {
    const points = d.map((v) => x(v).toFixed(1) + ',' + y(v).toFixed(1));
    return 'M' + points.join('L');
};

const lerp = (v0: number, v1: number, weight: number) => v0 + weight * (v1 - v0);

const zip = (a0: any[], a1: any[]) => a0.map((v, i) => [v, a1[i]]);
const DEG2RAD = Math.PI / 180;
export { zip, lerp, line, DEG2RAD };